   ...
   */
    ...
    public void FindThresholdSearchAlt(bool[,] visited, Cell start)
    {
        ...
        while (s.Count > 0)
        {
            ...
            //check if we haven't gone beyond the map
            if (x < 0 || z < 0 || x > gridWidth - 1 || z > gridHeight - 1)
            {
                continue;
            }
            if (!visited[cell.gridPositionZ, cell.gridPositionX])
            {
                ...
                {
                    Cell c = e.incident;
                    if (c.threshold || visited[c.gridPositionZ, c.gridPositionX] || cell.threshold)
                    {
                        continue;
                    }
                    if (c.zoneId != cell.zoneId)
    ...
    public void colorMap()
    {
        ...
        for (int i = 0; i < gridWidth; i++)
        {
            for (int j = 0; j < gridHeight; j++)
            {
                ...
                cube.transform.localScale = new Vector3(c.cellSize / 2f, 0, -1f * c.cellSize / 2f);
                //check state of cell  
